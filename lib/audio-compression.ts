...
export async function compressAudioFile(
  ...
  // Strategy: Reduce sample rate aggressively to fit under limit
  // Target: 44.1kHz or lower
  let targetSampleRate = 44100
  
  // If still too big, go lower
  const estimatedSize = duration * targetSampleRate * 2 * 2 // 16-bit stereo
  if (estimatedSize > maxBytes) {
    targetSampleRate = 22050 // Go to 22kHz
  }
