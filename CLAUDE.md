...
### Session 2026-06-11 (later) — Persistent IP limiter (Supabase wiring) + schema drift fix
- **Root cause found**: `init_ip_limiter()` never received a Supabase client AND the live `anonymous_sessions` table was missing 12 columns (incl. `ip_hash`) that the RPCs reference — `check_ip_limit` errored "column ip_hash does not exist" since forever, so the Supabase path NEVER worked; memory store carried everything (verified live via REST error-probes with anon key).
- **`ip_limiter.py`**: new `SupabaseRpcClient` — minimal async PostgREST RPC adapter on `aiohttp` (already in requirements; avoids the full supabase-py tree on the 512MB instance). Mirrors supabase-py's `rpc().execute()` shape; both RPC call sites now `await ... .execute()`. Both RPCs are SECURITY DEFINER → the public **anon key** suffices (no service key on Render).
