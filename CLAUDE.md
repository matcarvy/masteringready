...
**Git state (2026-07-13)**: `main` + `dev` both on `624343d`, pushed, build clean (next build + tsc). Untracked `content/` + 3 `content_queue` migrations belong to the content-creator feature; keep them out of unrelated commits. `CLAUDE-archive.md` and the two `docs/MR-*-Plan.md` files are untracked by choice.
...
### Master mode shipped (v7.5.0, 2026-07-13), NOT yet deployed

Built per `docs/MR-Master-Mode-Plan.md`. Every gate in its section 1.6 passes: py_compile, `next build`, `tsc --noEmit`, the 18 commercial masters score 83 to 98 (mean 92.1) where they used to score 26 to 29, the 6 controls behave, the crushed master lands at 55, a real master driven through both analysis paths returns a master verdict and a master CTA, and the PDF verdict matches the screen verdict in both profiles and both languages.

**Blocking deploy: migration `20260713000002_analyses_profile.sql` is written but NOT applied.** It adds `analyses.profile`. The frontend already writes the column. Applying it is a manual step and needs an explicit go (Hard Rule #8). Until it is applied, the insert will fail on the unknown column, so **do not push to `main` before running it.**
...
1. **Apply the migration, then deploy master mode.** Then watch the `remaster` CTA action in `cta_clicks`: a master scoring 60 to 84 is the best mastering-service lead the product can produce (they have a master, they are not happy with it, they have budget). If that band converts, point its CTA at the existing $97 Audit Stripe page in `~/masteringready-features/` rather than inventing a new SKU. Master mode itself stays free (RoEx gives stage detection away; the paid upgrade is Phase 2 reference comparison, which master mode unblocks).
